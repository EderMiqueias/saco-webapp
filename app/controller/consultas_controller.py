...
def get_consultas():
    ...
    results_1 = list(f.get_json() for f in dao_f.select_all())

    # g-2) Quais são nossos Funcionários que também são clientes?
    results_2 = list(fc.get_json() for fc in dao.select_fc())
    ...
    return {
        'message': 'success',
        'response': {
            "Funcionarios": results_1,
            "Funcionarios_clientes": results_2,
